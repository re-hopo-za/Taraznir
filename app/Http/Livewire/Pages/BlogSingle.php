...
class BlogSingle extends Component
{
    ...
    public function mount( $slug )
    {
        ...
        $this->blog = redisHandler('blog_'.$en_slug ,function () use($slug) {
        ...
        $this->categories = redisHandler( 'blog_categories' ,function (){
            return Category::where( 'model' ,'blog' )->get();
        });


        $categories = $this->blog->categories->modelKeys();
        $this->related = redisHandler( 'blogs_related_'.$en_slug,function () use($categories ,$blogs){
        ...
        if ( !$this->related->count() ) {
            $this->related = redisHandler( 'blog_not_related_'.$en_slug ,function () use($blogs){
